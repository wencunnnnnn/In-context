...
def main(args):
    ...
    # 推理场景下不再使用 DeepSpeed 启动多进程，而是依赖 transformers 的 device_map 做多卡分配
    set_seed(42)

    # 初始化分布式后端（单进程），DeepSpeed MoE 层内部的 all_to_all 通信需要这个
    ...
    # 使用 transformers 的 device_map 自动把模型切到多卡，避免与 DeepSpeed MoE kernel 注入冲突
    if args.cpu_only:
        device_map = None
    else:
        device_map = "auto"
    ...
    if args.cpu_only:
        model_engine = model.to(dtype=torch_dtype, device="cpu")
        input_device = "cpu"
    else:
        # 已由 device_map="auto" 自动将模型分配到多 GPU，上层统一通过 model_engine 调用
        model_engine = model

        # 关键修复：移除 accelerate 在 vision tower 和 SAM 子模块上安装的 hooks，
        # 否则 accelerate 会在内部子模块上拦截 forward，
        # 导致输入张量和权重在不同 GPU 上从而报 "GET was unable to find an engine" 错误。
        # 移除 hooks 后，手动将这些模块统一放到 cuda:0 上。

        # 1. CLIP vision tower
        remove_hook_from_submodules(vision_tower)
        vision_tower.to(dtype=torch_dtype, device="cuda:0")
        print(f"[INFO] Vision tower hooks removed, moved to device: cuda:0")

        # 2. SAM visual_model（image_encoder、prompt_encoder、mask_decoder）
        ...
        print(f"[INFO] SAM visual_model hooks removed, moved to device: cuda:0")

        # 3. text_hidden_fcs（用于从 hidden states 生成 seg embedding）
    ...
    model_engine.eval()
    if args.eval_seg:
        validate_seg(val_loader, model_engine, 0, args, tokenizer, fea_hooks=fea_hooks, input_device=input_device)
        exit()
    if args.eval_vqa:
        validate_vqa(val_loader, model_engine, 0, args, tokenizer, fea_hooks=fea_hooks, input_device=input_device)
        exit()
...
def validate_vqa(val_loader, model_engine, epoch, args, tokenizer, fea_hooks=None, input_device="cuda"):
...
def validate_seg(val_loader, model_engine, epoch, args, tokenizer, fea_hooks=None, input_device="cuda"):
